...
import { SentimentAnalysisResponse, EmotionData } from './gemini';

// Emozioni predefinite con loro caratteristiche
const emotionLibrary: Record<string, {
  description: string;
  color: string;
  keywords: string[];
  intensity: number;
 ...
 */
export function generateFallbackEmotionAnalysis(dream: string, story: string): SentimentAnalysisResponse {
  ...
  const emotions: EmotionData[] = selectedEmotions.map(emotionName => {
